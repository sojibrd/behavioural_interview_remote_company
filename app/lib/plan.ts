...
/** কাজের লেখায় `(ডক ০২.০২)` — `guide/`-এর কোন ডক; পাতা এই সাইটেই */
export type TaskSource = { kind: "doc"; code: string; label: string; title: string; href: string };
...
/** `(ডক ০২.০২)`, `(ডক ০৪.০১ · ডক ০৪.০৪)` — স্তর.ডক */
const DOC_GROUP_RE = /\((ডক\s+[০-৯0-9]+\.[০-৯0-9]+(?:\s*·\s*ডক\s+[০-৯0-9]+\.[০-৯0-9]+)*)\)/g;
const DOC_NUM_RE = /ডক\s+([০-৯0-9]+)\.([০-৯0-9]+)/g;
...
function parseSources(dayCode: string, raw: string): TaskSource[] {
  ...
  for (const group of raw.matchAll(DOC_GROUP_RE)) {
    for (const match of group[1].matchAll(DOC_NUM_RE)) {
      const doc = findGuideDoc(Number(toAsciiDigits(match[1])), Number(toAsciiDigits(match[2])));
      ...
      if (sources.some((item) => item.code === doc.code)) continue;
      sources.push({ kind: "doc", code: doc.code, label: doc.label, title: doc.title, href: `/doc/${doc.code}/` });
    }
  }
...
/** ডকের key (`"02-02"`) → plan-এর যে দিনের যে কাজে ডকটা আছে, ডকের পাতার জন্য */
