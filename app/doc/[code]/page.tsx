...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  return { title: doc ? `ডক ${doc.label} · ${doc.title}` : "ডক" };
}

/**
 * একটা ডক: এই পথে কোন দিনের কোন কাজে আসে, তারপর `guide/`-এর লেখা হুবহু। ডক পড়া
 * ইনপুট — কাজ হলো পড়ে বন্ধ করে নিজের story দিয়ে বলা।
 */
export default async function DocPage({ params }: { params: Promise<Params> }) {
  ...
  return (
    <>
      ...
      <Pager
        label="ডক"
        prev={prev && { href: `/doc/${prev.code}/`, label: `${prev.label} ${prev.title}` }}
        next={next && { href: `/doc/${next.code}/`, label: `${next.label} ${next.title}` }}
      />
    </>
  );
}
